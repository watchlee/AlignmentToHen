...
double arc_operation(int p1,int p2,int p3,int p4)
{
    ...
    if(base_mismatch(p1,p2)>=0&&base_mismatch(p3,p4)>=0)
    {
        return 0;
    }
...
int main(int argc,char* argv[])
{
    ...
    cout<<astr2<<endl;
    cout<<total<<endl;
    //const char *pdb_result= "/home/watchlee/Research_Programming/X3DNA/23-4L_SARA_FSCOR_structure/1MT4_A_to_1MSY_A/semi_result.php";
    const char *pdb_result= "/home/watchlee/result.php";
    write_data(score,pdb_result);


    //score=computation_for_sequence_global_ali();
    /*釋放記憶體*/
    free(alphabet_index);//來源 line:33
    return 0;
}

void write_profit_data()
{}
...
void write_data(double score,const char *path)
{
    ...
    file<<"$result_list[0][\"Nmat\"] = \"72\";"<<endl;
...
double computation_for_sequence_global_ali()
{
    ...
    for(int i=1;i<ssize;i++)
    {
        for(int j =1;j<ssize2;j++)
        {
            ...
            //if(seq1[i]==seq2[j])
            //    cout<<"match!"<<endl;
            match_cost=S[i-1][j-1]+base_mismatch(i-1,j-1)*w_m;
    ...
    */
    ...
    while(i>0 || j>0)
    {
        if(i>0 && j>0 && S[i][j]==S[i-1][j-1]+base_mismatch(i-1,j-1)*w_m)
        {
...
void traceback()
{
  ...
  while(!ranges.empty())
      ...
      else if (l1<=r1 && l2<=r2)
	{
	  ...
	  if (arc1[a1]==')' && arc2[a2]==')')
	    {
	      double w=M[L1[I1[a1]]-l1][L2[I2[a2]]-l2]+D[I1[a1]][I2[a2]]+arc_operation(L1[I1[a1]],L2[I2[a2]],a1,a2);//(base_mismatch(L1[I1[a1]],L2[I2[a2]])+base_mismatch(a1,a2))*0.5*w_am;
	      if (fabs(M[k][l]-w)<eps)
		{
		  int i1=L1[I1[a1]];              // left arc ends
		  int j1=L2[I2[a2]];
		  
		  double edge_weight=0.5*arc_operation(L1[I1[a1]],L2[I2[a2]],a1,a2); //(base_mismatch(L1[I1[a1]],L2[I2[a2]])+base_mismatch(a1,a2))*0.5*w_am;
        ...
        for(int k=1;k<R1[i]-L1[i];k++)
            for(int l=1;l<R2[j]-L2[j];l++)
            {
                ...
                if(arc1[a1]==')' && arc2[a2]==')')
                {
                    ...
                    v4 = M[leftpoint - L1[i]-1][leftpoint2 - L2[j]-1]+D[I1[a1]][I2[a2]]+arc_operation(L1[I1[a1]],L2[I2[a2]],R1[I1[a1]],R2[I2[a2]]);//(base_mismatch(L1[I1[a1]],L2[I2[a2]])+base_mismatch(R1[I1[a1]],R2[I2[a2]]))*0.5*w_am;
                }
                M[k][l]=min4(v1,v2,v3,v4);
                //M[k][l]=max4(v1,v2,v3,v4);
            }
    ...
    for (int k=1;k<=arc1.size();k++)
        for (int l=1;l<=arc2.size();l++)
        {
            ...
            if(arc1[k-1]==')'&& arc2[l-1]==')')
            {
                ...
                +arc_operation(L1[I1[k-1]],L2[I2[l-1]],R1[I1[k-1]],R2[I2[l-1]]);//(base_mismatch(L1[I1[k-1]],L2[I2[l-1]])+base_mismatch(R1[I1[k-1]],R2[I2[l-1]]))*0.5*w_am;
...
void determine_MAX(double *arc_max,double *seq_max)
{
    ...
    //r = 2 , k = 4 base mismatch = 2 * seq_max - base_mismatch 
    //r = 2 , k = 6 arc altering= 2 * arc_max - base_match or base_mismatch - w_b
    //r = 2 , k = 7 arc breaking= arc_max + seq_max - base_mismatch - w_b/2 
    //上兩項合併 = 2*seq_max + base_mismatch() 加入到v3 = 
