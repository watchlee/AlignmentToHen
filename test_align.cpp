    ...
    else
    {
        if((base_matching(p1,p2)<0 && base_matching(p3,p4)>=0)||(base_matching(p1,p2)>=0&&base_matching(p3,p4)<0))
        {
            if((base_matching(p1,p2)+base_matching(p3,p4))>=0)
            {
                ...
                printf("%d %d\n",base_matching(p1,p2),base_matching(p3,p4));
                return (base_matching(p1,p2)+base_matching(p3,p4))*1.5;
            } 
            else
            {
...
int main(int argc,char* argv[])
{
    ...
    if(argc!=3)
    {
        /*--------------------可能會因為php格式的關係 造成score不同 ex:特殊字元 php需要用\再額外表示 但c++不用*/
        ...
        //pdb_compare_path= "./Test_for_GLOBAL/long_input.php";
        pdb_compare_path= "./Test_for_GLOBAL/1J5E_A_to_1Q7Y_A.php";
...
int BinarySearch(char character,IndexMatrix *array,int length)
{
    ...
    while(low<=high)
    {
        ...
        {
            printf("%c %d\n",array[mid].alphabet,array[mid].pos);
...
int base_matching(int pos1,int pos2)
{
    ...
    printf("%c %c = %d index_x = %d index_y = %d \n",seq1[pos1],seq2[pos2],scoring_matrix[index_x][index_y],index_x,index_y); 
